...
double CUDACrossEntropy::LaunchCalcInitScoreKernel() const {
  ...
  if (cuda_weights_ == nullptr) {
    ...
    ShuffleReduceSumGlobal<label_t, double>(cuda_labels_, static_cast<size_t>(num_data_), cuda_reduce_sum_buffer_);
    CopyFromCUDADeviceToHost<double>(&suml, cuda_reduce_sum_buffer_, 1, __FILE__, __LINE__);
  } else {
    ShuffleReduceDotProductGlobal<label_t, label_t, double>(cuda_labels_, cuda_weights_, static_cast<size_t>(num_data_), cuda_reduce_sum_buffer_);
    CopyFromCUDADeviceToHost<double>(&suml, cuda_reduce_sum_buffer_, 1, __FILE__, __LINE__);
    ShuffleReduceSumGlobal<label_t, double>(cuda_weights_, static_cast<size_t>(num_data_), cuda_reduce_sum_buffer_);
    CopyFromCUDADeviceToHost<double>(&sumw, cuda_reduce_sum_buffer_, 1, __FILE__, __LINE__);
  }
