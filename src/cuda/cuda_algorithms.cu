...
template <typename T>
__global__ void ShufflePrefixSumGlobalKernelDataPartition(T* values, size_t len, T* block_prefix_sum_buffer) {
  __shared__ T shared_mem_buffer[32];
  const size_t index = static_cast<size_t>(threadIdx.x + blockIdx.x * blockDim.x);
  T value = 0;
...
__global__ void ShufflePrefixSumGlobalKernelDataPartition(uint64_t* values, size_t len, uint64_t* block_prefix_sum_buffer) {
  ...
  const uint64_t prefix_sum_value = ShufflePrefixSum<uint64_t>(value, shared_mem_buffer);
  if (index < len) {
    values[index] = prefix_sum_value;
  }
  if (threadIdx.x == blockDim.x - 1) {
...
__global__ void ShufflePrefixSumGlobalReduceBlockKernel(uint64_t* block_prefix_sum_buffer, int num_blocks) {
  __shared__ uint64_t shared_mem_buffer[32];
  const int num_blocks_per_thread = (num_blocks + GLOBAL_PREFIX_SUM_BLOCK_SIZE - 2) / (GLOBAL_PREFIX_SUM_BLOCK_SIZE - 1);
  int thread_block_start = threadIdx.x == 0 ? 0 : (threadIdx.x - 1) * num_blocks_per_thread;
  int thread_block_end = threadIdx.x == 0 ? 0 : min(thread_block_start + num_blocks_per_thread, num_blocks);
  ...
  base = ShufflePrefixSum<uint64_t>(base, shared_mem_buffer);
  thread_block_start = threadIdx.x == blockDim.x - 1 ? 0 : threadIdx.x * num_blocks_per_thread;
  thread_block_end = threadIdx.x == blockDim.x - 1 ? 0 : min(thread_block_start + num_blocks_per_thread, num_blocks);
  for (int block_index = thread_block_start + 1; block_index < thread_block_end; ++block_index) {
    block_prefix_sum_buffer[block_index] += block_prefix_sum_buffer[block_index - 1];
...
template <typename T>
__global__ void ShufflePrefixSumGlobalAddBaseDataPartition(size_t len, const T* block_prefix_sum_buffer, T* values) {
  const T to_left_count = ((block_prefix_sum_buffer[gridDim.x - 1] & 0x00000000ffffffff) << 32);
  const T base = blockIdx.x == 0 ? to_left_count : (block_prefix_sum_buffer[blockIdx.x - 1] + to_left_count);
  const size_t index = static_cast<size_t>(threadIdx.x + blockIdx.x * blockDim.x);
  if (index < len) {
    values[index] += base;
  }
...
void ShufflePrefixSumGlobalInnerAsync(T* values, size_t len, T* block_prefix_sum_buffer, cudaStream_t cuda_stream) {
  const int num_blocks = (static_cast<int>(len) + GLOBAL_PREFIX_SUM_BLOCK_SIZE - 1) / GLOBAL_PREFIX_SUM_BLOCK_SIZE;
  ShufflePrefixSumGlobalKernelDataPartition<<<num_blocks, GLOBAL_PREFIX_SUM_BLOCK_SIZE, 0, cuda_stream>>>(values, len, block_prefix_sum_buffer);
  ShufflePrefixSumGlobalReduceBlockKernel<<<1, GLOBAL_PREFIX_SUM_BLOCK_SIZE, 0, cuda_stream>>>(block_prefix_sum_buffer, num_blocks);
  ShufflePrefixSumGlobalAddBaseDataPartition<<<num_blocks, GLOBAL_PREFIX_SUM_BLOCK_SIZE, 0, cuda_stream>>>(len, block_prefix_sum_buffer, values);
}
...
template <>
void ShufflePrefixSumGlobalAysnc(uint64_t* values, size_t len, uint64_t* block_prefix_sum_buffer, cudaStream_t cuda_stream) {
  ShufflePrefixSumGlobalInnerAsync<uint64_t>(values, len, block_prefix_sum_buffer, cuda_stream);
}
...
__global__ void ShuffleReduceDotProductGlobalKernel(const VAL_A_T* a, const VAL_B_T* b, const data_size_t num_value, REDUCE_T* block_buffer) {
  __shared__ REDUCE_T shared_buffer[32];
  const data_size_t data_index = static_cast<data_size_t>(blockIdx.x * blockDim.x + threadIdx.x); 
...
template <typename VAL_A_T, typename VAL_B_T, typename REDUCE_T>
void ShuffleReduceDotProductGlobalInner(const VAL_A_T* a, const VAL_B_T* b, size_t n, REDUCE_T* block_buffer) {
  const data_size_t num_value = static_cast<data_size_t>(n);
  const data_size_t num_blocks = (num_value + GLOBAL_PREFIX_SUM_BLOCK_SIZE - 1) / GLOBAL_PREFIX_SUM_BLOCK_SIZE;
  ShuffleReduceDotProductGlobalKernel<VAL_A_T, VAL_B_T, REDUCE_T><<<num_blocks, GLOBAL_PREFIX_SUM_BLOCK_SIZE>>>(a, b, num_value, block_buffer);
  BlockReduceSum<REDUCE_T><<<1, GLOBAL_PREFIX_SUM_BLOCK_SIZE>>>(block_buffer, num_blocks);
}

template <>
void ShuffleReduceDotProductGlobal<label_t, label_t, double>(const label_t* a, const label_t* b, size_t n, double* block_buffer) {
  ShuffleReduceDotProductGlobalInner<label_t, label_t, double>(a, b, n, block_buffer);
}
