...
void NewCUDATreeLearner::AddPredictionToScore(const Tree* /*tree*/, double* score) const {
  cuda_data_partition_->UpdateTrainScore(config_->learning_rate, score);
}
...
Tree* NewCUDATreeLearner::BuildTree(const int num_leaves) {
  ...
  for (int i = 0; i < num_leaves; ++i) {
    Log::Warning("leaf_value[%d] = %f inner_feature_index[%d] = %d threshold[%d] = %d from CPU tree builder", i, tree->LeafOutput(i), i, inner_feature_index[i], i, threshold[i]);
  }
  Log::Warning("num_leaves = %d", tree->num_leaves());
  return new CUDATree(tree.release());
}
