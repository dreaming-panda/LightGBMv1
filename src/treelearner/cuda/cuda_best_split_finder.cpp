...
void CUDABestSplitFinder::Init() {
  ...
  AllocateCUDAMemory<int>(&cuda_best_split_info_buffer_, 10, __FILE__, __LINE__);
  ...
  CUDASUCCESS_OR_FATAL(cudaMallocHost(&host_leaf_split_info_buffer_, 10 * sizeof(int)));
}
...
void CUDABestSplitFinder::FindBestSplitsForLeaf(
  ...
  if (grad_scale != nullptr && hess_scale != nullptr) {
    LaunchFindBestSplitsDiscretizedForLeafKernel(
