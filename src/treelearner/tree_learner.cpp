...
TreeLearner* TreeLearner::CreateTreeLearner(const std::string& learner_type, const std::string& device_type, const Config* config) {
  ...
  if (device_type == std::string("cpu")) {
    ...
    } else if(learner_type == std::string("symmetric_share_multi")) {
      Log::Warning("use symmetric tree share multi");
      return new SymmetricTreeShareThresholdMultiFeatureLearner(config);
    } else if(learner_type == std::string("symmetric_cycle")) {
      Log::Warning("use symmetric cycle tree");
      return new SymmetricTreeShareThresholdRefreshLearner(config);
