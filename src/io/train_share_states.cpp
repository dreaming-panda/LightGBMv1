...
void MultiValBinWrapper::MixHistMove(const std::vector<int_hist_t,
  Common::AlignmentAllocator<int_hist_t, kAlignedSize>>& int_hist_buf,
  const std::vector<hist_t, Common::AlignmentAllocator<hist_t, kAlignedSize>>& hist_buf) {
  if (!is_use_subcol_) {
    const int_hist_t* int_src = int_hist_buf.data();
    const hist_t* src = hist_buf.data();
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < num_bin_; ++i) {
      origin_hist_data_[2 * i] = src[i];
      origin_hist_data_[2 * i + 1] = int_src[i] * hess_scale;
    }
  } else {
    const int_hist_t* int_src = int_hist_buf.data();
    const hist_t* src = hist_buf.data();
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < static_cast<int>(hist_move_src_.size()); ++i) {
      const int_hist_t* int_src_ptr = int_src + hist_move_src_[i] / 2;
      const hist_t* src_ptr = src + hist_move_src_[i] / 2;
      hist_t* dst_ptr = origin_hist_data_ + hist_move_dest_[i];
      for (int j = 0; j < static_cast<int>(hist_move_size_[i]) / 2; ++j) {
        dst_ptr[2 * j] = src_ptr[j];
        dst_ptr[2 * j + 1] = hess_scale * int_src_ptr[j];
      }
    }
  }
}
...
void MultiValBinWrapper::MixHistMerge(std::vector<int_hist_t,
  Common::AlignmentAllocator<int_hist_t, kAlignedSize>>* int_hist_buf,
  ...
  Threading::BlockInfo<data_size_t>(num_threads_, num_bin_, 512, &n_bin_block,
                                  &bin_block_size);
  int_hist_t* int_dst = int_hist_buf->data();
  hist_t* dst = hist_buf->data();
  #pragma omp parallel for schedule(static, 1) num_threads(num_threads_)
  for (int t = 0; t < n_bin_block; ++t) {
    const int start = t * bin_block_size;
    const int end = std::min(start + bin_block_size, num_bin_);
    for (int tid = 1; tid < n_data_block_; ++tid) {
      auto int_src_ptr = int_hist_buf->data() + static_cast<size_t>(num_bin_aligned_) * tid;
      auto src_ptr = hist_buf->data() + static_cast<size_t>(num_bin_aligned_) * tid;
      for (int i = start; i < end; ++i) {
        dst[i] += src_ptr[i];
        int_dst[i] += int_src_ptr[i];
      }
    }
  }
}
...
void MultiValBinWrapper::ResizeMixHistBuf(std::vector<int_hist_t,
  Common::AlignmentAllocator<int_hist_t, kAlignedSize>>* int_hist_buf,
  std::vector<hist_t, Common::AlignmentAllocator<hist_t, kAlignedSize>>* hist_buf,
  MultiValBin* sub_multi_val_bin,
  hist_t* origin_hist_data) {
