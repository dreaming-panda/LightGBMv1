...
void CUDARowData::GetDenseDataPartitioned(const BIN_TYPE* row_wise_data, std::vector<BIN_TYPE>* partitioned_data) {
  ...
  Threading::For<data_size_t>(0, num_data_, 512,
    [this, num_total_columns, row_wise_data, out_data] (int /*thread_index*/, data_size_t start, data_size_t end) {
      for (size_t i = 0; i < feature_partition_column_index_offsets_.size() - 1; ++i) {
        const size_t num_prev_columns = static_cast<size_t>(feature_partition_column_index_offsets_[i]);
        const size_t offset = static_cast<size_t>(num_data_) * num_prev_columns;
        ...
        for (data_size_t data_index = start; data_index < end; ++data_index) {
          ...
          const size_t read_data_offset = static_cast<size_t>(data_index) * static_cast<size_t>(num_total_columns);
          for (size_t column_index = 0; column_index < static_cast<size_t>(num_columns_in_cur_partition); ++column_index) {
            const size_t true_column_index = read_data_offset + column_index + static_cast<size_t>(partition_column_start);
