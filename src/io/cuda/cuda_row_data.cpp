...
#include "cuda_row_data.hpp"

namespace LightGBM {

CUDARowData::CUDARowData(const Config* config):
  max_num_bin_per_partition_(6144) {
  num_threads_ = config->num_threads;
}

CUDARowData::~CUDARowData() {}

void CUDARowData::DivideCUDAFeatureGroups(const Dataset* train_data, const TrainingShareStates* share_state) {
  ...
  for (int feature_group_index = 0; feature_group_index < num_feature_groups; ++feature_group_index) {
    if (!train_data->IsMultiGroup(feature_group_index)) {
      ...
      if (num_bin_in_dense_group > static_cast<uint32_t>(max_num_bin_per_partition_)) {
      ...
      if (cur_hist_num_bin > static_cast<uint32_t>(max_num_bin_per_partition_)) {
  ...
  InitCUDAMemoryFromHostMemoryOuter<uint32_t>(&cuda_column_hist_offsets_full_,
...
void CUDARowData::InitCUDADataInner(const size_t total_size, const BIN_TYPE* host_data, const ROW_PTR_TYPE* host_row_ptr) {
  BIN_TYPE* cuda_data = nullptr;
  if (!is_row_wise_sparse_) {
    std::vector<BIN_TYPE> partitioned_data;
    GetDenseDataPartitioned<BIN_TYPE>(host_data, &partitioned_data);
    InitCUDAMemoryFromHostMemoryOuter<BIN_TYPE>(&cuda_data,
                                           partitioned_data.data(),
                                           total_size,
                                           __FILE__,
                                           __LINE__);
  } else {
    ROW_PTR_TYPE* cuda_row_ptr = nullptr;
    ROW_PTR_TYPE* cuda_partition_ptr = nullptr;
    std::vector<std::vector<BIN_TYPE>> partitioned_data;
    ...
    InitCUDAMemoryFromHostMemoryOuter<ROW_PTR_TYPE>(&cuda_partition_ptr,
                                               partition_ptr.data(),
                                               partition_ptr.size(),
                                               __FILE__,
                                               __LINE__);
    ...
    for (size_t i = 0; i < partitioned_data.size(); ++i) {
      ...
      CopyFromHostToCUDADeviceOuter<BIN_TYPE>(cuda_data + partition_ptr[i],
                                         data_for_this_partition.data(),
                                         data_for_this_partition.size(),
                                         __FILE__,
                                         __LINE__);
      CopyFromHostToCUDADeviceOuter<ROW_PTR_TYPE>(cuda_row_ptr + i * (num_data_ + 1),
                                             data_ptr_for_this_partition.data(),
                                             data_ptr_for_this_partition.size(),
                                             __FILE__,
                                             __LINE__);
    }
    cuda_partition_ptr_ = reinterpret_cast<void*>(cuda_partition_ptr);
    cuda_row_ptr_ = reinterpret_cast<void*>(cuda_row_ptr);
  }
  cuda_data_by_row_ = reinterpret_cast<void*>(cuda_data);
}

void CUDARowData::InitCUDAData(const TrainingShareStates* train_share_states) {
  size_t total_size = 0;
  const void* host_row_ptr = nullptr;
  const void* host_data_ptr = train_share_states->GetRowWiseData(
    &cuda_data_by_row_bit_type_, &total_size, &is_row_wise_sparse_, &host_row_ptr, &cuda_row_ptr_bit_type_);
  if (cuda_data_by_row_bit_type_ == 8) {
    const uint8_t* true_host_data_ptr = reinterpret_cast<const uint8_t*>(host_data_ptr);
    if (cuda_row_ptr_bit_type_ == 16) {
      const uint16_t* true_host_row_ptr = reinterpret_cast<const uint16_t*>(host_data_ptr);
      InitCUDADataInner<uint8_t, uint16_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 32) {
      const uint32_t* true_host_row_ptr = reinterpret_cast<const uint32_t*>(host_data_ptr);
      InitCUDADataInner<uint8_t, uint32_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 64) {
      const uint64_t* true_host_row_ptr = reinterpret_cast<const uint64_t*>(host_data_ptr);
      InitCUDADataInner<uint8_t, uint64_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else {
      Log::Fatal("Unknow row ptr bit type %d", cuda_row_ptr_bit_type_);
    }
  } else if (cuda_data_by_row_bit_type_ == 16) {
    const uint16_t* true_host_data_ptr = reinterpret_cast<const uint16_t*>(host_data_ptr);
    if (cuda_row_ptr_bit_type_ == 16) {
      const uint16_t* true_host_row_ptr = reinterpret_cast<const uint16_t*>(host_data_ptr);
      InitCUDADataInner<uint16_t, uint16_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 32) {
      const uint32_t* true_host_row_ptr = reinterpret_cast<const uint32_t*>(host_data_ptr);
      InitCUDADataInner<uint16_t, uint32_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 64) {
      const uint64_t* true_host_row_ptr = reinterpret_cast<const uint64_t*>(host_data_ptr);
      InitCUDADataInner<uint16_t, uint64_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else {
      Log::Fatal("Unknow row ptr bit type %d", cuda_row_ptr_bit_type_);
    }
  } else if (cuda_data_by_row_bit_type_ == 32) {
    const uint32_t* true_host_data_ptr = reinterpret_cast<const uint32_t*>(host_data_ptr);
    if (cuda_row_ptr_bit_type_ == 16) {
      const uint16_t* true_host_row_ptr = reinterpret_cast<const uint16_t*>(host_data_ptr);
      InitCUDADataInner<uint32_t, uint16_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 32) {
      const uint32_t* true_host_row_ptr = reinterpret_cast<const uint32_t*>(host_data_ptr);
      InitCUDADataInner<uint32_t, uint32_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else if (cuda_row_ptr_bit_type_ == 64) {
      const uint64_t* true_host_row_ptr = reinterpret_cast<const uint64_t*>(host_data_ptr);
      InitCUDADataInner<uint32_t, uint64_t>(total_size, true_host_data_ptr, true_host_row_ptr);
    } else {
      Log::Fatal("Unknow row ptr bit type %d", cuda_row_ptr_bit_type_);
    }
  } else {
    Log::Fatal("Unknow bit type = %d", cuda_data_by_row_bit_type_);
  }
  SynchronizeCUDADeviceOuter(__FILE__, __LINE__);
}

void CUDARowData::Init(const Dataset* train_data, const TrainingShareStates* train_share_states) {
  max_num_bin_per_partition_ = 6144;
  num_data_ = train_data->num_data();
  num_features_ = train_data->num_features();

  DivideCUDAFeatureGroups(train_data, train_share_states);

  cuda_data_by_row_bit_type_ = 0;
  cuda_row_ptr_bit_type_ = 0;
  cuda_data_by_row_ = nullptr;
  cuda_row_ptr_ = nullptr;

  InitCUDAData(train_share_states);
}
